...
async def search_title(request: Request, title: str = None, page: int = 1):
    ...
    try:
        ...
        return templates.TemplateResponse("results.html", {
            "request": request, "results": results, "search_term": title,
            "page": page, "total_count": total_count, **common_data
        })
...
async def search_filter_route(request: Request, category: str = None, year_from: int = None, year_to: int = None, page: int = 1):
    ...
    try:
        ...
        search_label = f"Фильтр: {genre or 'Все'} ({year_from or ''}-{year_to or ''})"
        
        if search_label and search_label.strip():
        ...
        from app.core.templates import templates
        return templates.TemplateResponse("results.html", {
            "request": request, "results": results, "search_term": search_label,
            "page": page, "total_count": total_count, "category": genre, "year_from": year_from, "year_to": year_to,
            **common_data
        })
    except Exception as e:
...
async def genre_page(request: Request, genre_name: str, page: int = 1, year_from: int = None, year_to: int = None):
    ...
    try:
        ...
        common_data = get_common_data()
        search_label = f"Жанр: {genre_name} ({year_from or ''}-{year_to or ''})"
        save_search_query(search_label)
        ...
        return templates.TemplateResponse("results.html", {
            ...
            "category": genre_name, "year_from": year_from, "year_to": year_to,
            **common_data
        })
    except Exception as e:
